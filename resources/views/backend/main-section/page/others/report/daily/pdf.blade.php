...
<div style="position:absolute;top:0.40in;left:4.50in;width:2.30in;line-height:0.23in;"><span style="font-style:normal;font-weight:normal;font-size:15pt;font-family:Segoe UI;color:#ff1100">Daily & Monthly Report</span><span style="font-style:normal;font-weight:normal;font-size:17pt;font-family:Segoe UI;color:#212529"> </span><br/></SPAN></div>
...
<br/><br/><br/><br/><br/><br/><br/><br/><br/>
        ...
        <tbody>
		  ...
		  <tr>
			<td align="center">#{{$details->invoice_no}}</td>
			<td>{{$details['payment']['customer']['name']}}</td>
            <td>{{date('d-Y',strtotime($details->date))}}</td>
			<td>BDT. {{$details['payment']['paid_amount']}}</td>
            <td>BDT. {{$details['payment']['due_amount']}}</td>
			<td>BDT. {{$details['payment']['total_amount']}}</td>
		  </tr>
		  @php
		  $total_sum += $details['payment']['total_amount'];
          $total_paid += $details['payment']['paid_amount'];
          $total_due += $details['payment']['due_amount'];
		  @endphp
		  @endforeach
        </tbody>
		<tbody id="mrs">
			  <tr>
				<th colspan="3"></th>
                <td style="background: rgb(218, 120, 218)">BDT. {{$total_paid}}</td>
				<td style="background: rgb(196, 196, 196)">BDT. {{$total_due}}</td>
